...
Routing prototype only. Wider power traces currently fail board-edge DRC; do not order these outputs. See TS_ISSUES.md for reproductions and cropped snapshots.

- **Keepout workaround resolved:** core supports `<keepout />`; the documented `<pcbkeepout />` name was the problem. This branch emits two-layer IMU and antenna-footprint exclusions. The broader antenna assembly-clearance volume still needs mechanical validation.
- **Routing blockers:** pill-hole DRC needs the committed conservative dependency patch; net widths require explicit traces; wider-power autorouting produces off-board copper. See TS_ISSUES.md.
